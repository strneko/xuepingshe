...
interface TeacherHistoryCoursesProps {
  teacherId: string;
...
export default function TeacherHistoryCourses({ teacherId, initialHistoryScores }: TeacherHistoryCoursesProps) {
  ...
  const [items, setItems] = useState<HistoryScoreItem[]>(initialHistoryScores.items);
  const [nextCursor, setNextCursor] = useState<string | null>(initialHistoryScores.nextCursor);
  const [hasMore, setHasMore] = useState(initialHistoryScores.hasMore);
  ...
  const updateQuery = useCallback(
    (nextView: ViewType, nextGranularity: ScoreHistoryGranularity) => {
  ...
  const fetchPage = useCallback(
    async (cursor: string | null, replace: boolean) => {
      ...
      try {
        ...
        const response = await fetch(`/api/teachers/${teacherId}/score-history?${params.toString()}`, {
          method: "GET",
        });
  ...
  }, [fetchPage, granularity, teacherId]);
        ...
        <ButtonGroup>
          {VIEW_OPTIONS.map((option) => (
            <Button
              ...
              onClick={() => updateQuery(option.key, granularity)}
            >
              {option.label}
            </Button>
          ))}
