...
export default function CourseTabs({ announcements, resources }: CourseTabsProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  const rawTab = searchParams.get("tab");
  const activeTab: "announcement" | "resource" = rawTab === "resource" ? "resource" : "announcement";
  ...
  const totalPages = activeTab === "announcement" ? totalAnnouncementPages : totalResourcePages;
  const currentPage = Math.min(requestedPage, totalPages);

  const updateQuery = (nextTab: "announcement" | "resource", nextPage: number) => {
  ...
  const switchTab = (tab: "announcement" | "resource") => {
    updateQuery(tab, 1);
  };
